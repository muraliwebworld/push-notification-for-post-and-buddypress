    ...
    class PNFPB_avatar_change_notification_class
    {
        public function PNFPB_avatar_change_notification(
			...
			if (
				...
				$pnfpb_titletext = get_option("pnfpb_ic_fcm_avatar_change_text");
				...
				$pnfpb_contenttext = get_option("pnfpb_ic_fcm_avatar_change_content");
				...
				if (function_exists("bp_members_get_user_url")) {
					$pnfpb_messageurl = esc_url(bp_members_get_user_url($pnfpb_user_id));
				} else {
					$pnfpb_messageurl = esc_url(bp_core_get_user_domain($pnfpb_user_id));
				}
				...
				if ($pnfpb_webpush_option === '1' || $pnfpb_webpush_option === '2' || $pnfpb_webpush_firebase === '1') {
					...
					if (count($pnfpb_target_deviceid_values) > 0) {
						...
						$pnfpb_avatar_change_subscription_count = 0;

						if (get_option('pnfpb_avatarchange_subscription_count') !== false && 
							get_option('pnfpb_general_subscription_count') !== false &&
							(get_option('pnfpb_avatarchange_subscription_count') > 0 || get_option('pnfpb_general_subscription_count') > 0)) {
							$pnfpb_avatar_change_subscription_count = intval(get_option('pnfpb_avatarchange_subscription_count'))+intval(get_option('pnfpb_general_subscription_count'));
						}						
						...
						$PNFPB_WP_web_push_notification_class_obj->PNFPB_web_push_notification(
							...
							"",
							0,
							$pnfpb_avatar_change_subscription_count
						);						
					}
