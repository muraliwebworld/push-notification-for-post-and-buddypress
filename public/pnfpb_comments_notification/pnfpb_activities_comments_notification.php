...
if (!class_exists("PNFPB_activities_comments_notification_class")) {
    ...
    {
        public function PNFPB_activities_comments_notification(
			$comment_id = null,
            $params = null,
            $activity = null,
            $sendschedule = "no"
			...
			if (
				...
				if ($comment_id === null) {
					$recent_comments = $wpdb->get_results(
						$wpdb->prepare(
							"SELECT user_id, content, item_id, id, date_recorded FROM %i WHERE TYPE = %s ORDER BY date_recorded DESC LIMIT 1",
							$table_name_activity,
							"activity_comment"
						)
					);
					...
					foreach ($recent_comments as $recent_comment) {
						...
						$localactivitycontent = substr(
							stripslashes(wp_strip_all_tags($recent_comment->content)),
							0,
							80
						);
						...
						$activity_details = $wpdb->get_results(
							$wpdb->prepare(
								"SELECT user_id, content, item_id, id, date_recorded FROM %i WHERE id = %d AND TYPE = %s ORDER BY date_recorded DESC LIMIT 1",
								$table_name_activity,
								$activity_id,
								"activity_update"
							)
				...
				if (($webpush_option === '1' || $webpush_option === '2' || $webpush_firebase === '1') && $activity_id > 0) {
					
					if (get_option("pnfpb_ic_fcm_buddypress_comments_radio_enable") !== "2" ) {					
					
						$target_deviceid_values = $wpdb->get_results(
							$wpdb->prepare(
								"SELECT * FROM %i WHERE device_id NOT LIKE %s AND web_auth <> %s AND web_256 <> %s AND subscription_auth_token <> %s AND (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,3,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",
								$table_name,
								"%!!%",
								"","",""
							)
						...
						if (get_option("pnfpb_ic_fcm_buddypress_comments_radio_enable") === "2" ) {
							$target_deviceid_values = $wpdb->get_results(
								$wpdb->prepare(
									"SELECT * FROM %i WHERE device_id NOT LIKE %s AND userid = %d AND web_auth <> %s AND web_256 <> %s AND subscription_auth_token <> %s AND (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,4,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",
									$table_name,
									"%!!%",
					...
					if (get_option("pnfpb_progressier_push") === "1") {
						if (
							...
							$target_userid_array_values = $wpdb->get_col(
								$wpdb->prepare(
									"SELECT device_id FROM %i WHERE device_id LIKE %s AND (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,3,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",
									$table_name,
									"%progressier%"
								)
							);
						...
						if (
							...
							$target_userid_array_values = $wpdb->get_col(
								$wpdb->prepare(
									"SELECT device_id FROM %i WHERE userid = %d AND device_id LIKE %s AND (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,4,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",
									$table_name,
									$authoractivityuserid,
									"%progressier%"
								)
					...
					if (get_option("pnfpb_webtoapp_push") === "1") {
						if (
							...
							$target_userid_array_values = $wpdb->get_col(
								$wpdb->prepare(
									"SELECT device_id FROM %i WHERE (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,3,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",
									$table_name
								)
							);
						}
						...
						if (
							...
							$target_userid_array_values = $wpdb->get_col(
								$wpdb->prepare(
									"SELECT device_id FROM %i WHERE userid = %d AND  (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,4,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",
									$table_name,
									$authoractivityuserid
								)
							);
					...
					if (
						...
						if (function_exists("bb_activity_is_item_favorite")) {
							...
							$reacted_users = $wpdb->get_results(
								$wpdb->prepare(
									"SELECT id,user_id,reaction_id,item_type,item_id FROM %i WHERE item_id = %d AND item_type = %s",
									$table_name_bbreactions,
									$activity_id,
									"activity"
								)
							);
							...
							if (count($liked_users) > 0) {
								...
								if (
									...
									$target_userid_array_values = $wpdb->get_col(
										$wpdb->prepare(
											"SELECT userid FROM %i WHERE userid IN ($liked_user_ids_implArray) AND device_id LIKE %s AND (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,3,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",
											$table_name,
											"%onesignal%"
										)
									);
								...
								if (
									...
									$target_userid_array_values = $wpdb->get_col(
										$wpdb->prepare(
											"SELECT userid FROM %i WHERE userid = %d AND device_id LIKE %s AND (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,4,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",
											$table_name,
											$authoractivityuserid,
											"%onesignal%"
										)
							...
							$target_userid_array_values = $wpdb->get_col(
								$wpdb->prepare(
									"SELECT userid FROM %i WHERE device_id LIKE %d AND (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,3,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",
									$table_name,
									"%onesignal%"
								)
							);

							$target_myactivities_userid_array_values = $wpdb->get_col(
								$wpdb->prepare(
									"SELECT userid FROM %i WHERE userid = %d AND device_id LIKE %s AND (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,4,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",
									$table_name,
									$authoractivityuserid,
							...
							$mergeduserids_array = array_merge(
								$target_userid_array,
								$target_myactivities_userid_array
							);
						...
						if (get_option("pnfpb_progressier_push") !== "1") {
							...
							if (
								...
								if (count($liked_user_ids) <= 0 && !$buddyboss_pnfpb) {
									$deviceidsmyactivities = $wpdb->get_col(
										$wpdb->prepare(
											"SELECT DISTINCT SUBSTRING_INDEX(device_id, '!!', 1) FROM %i WHERE userid = %d AND device_id NOT LIKE %s AND device_id NOT LIKE %s AND device_id NOT LIKE %s AND (SUBSTRING(subscription_option,1,1) != '0' OR SUBSTRING(subscription_option,4,1) = '1' )",
											$table_name,
											$authoractivityuserid,
											"%webview%",
											"%!!%",
									...
									$deviceidsmyactivitieswebview = $wpdb->get_col(
										$wpdb->prepare(
											"SELECT DISTINCT SUBSTRING_INDEX(device_id, '!!', 1) FROM %i WHERE userid = %d AND device_id LIKE %s AND device_id NOT LIKE %s AND (SUBSTRING(subscription_option,1,1) != '0' OR SUBSTRING(subscription_option,4,1) = '1' )",
											$table_name,
											$authoractivityuserid,
											"%webview%",
											"%@N%"
							...
							$mergeddeviceidswebview = array_merge(
								$deviceidswebview,
								$deviceidsmyactivitieswebview
							);
