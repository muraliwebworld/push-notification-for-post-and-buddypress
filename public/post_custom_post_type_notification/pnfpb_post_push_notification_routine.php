...
if (
    ...
    (!isset($_POST["pnfpb_push_notification_send_checkbox"]) &&
        !isset($_POST["pnfpb_push_notification_schedule_checkbox"]))
) {
    return;
}
...
if (
	(!isset($_POST["pnfpb_push_notification_schedule_checkbox"]) ||
	(isset($_POST["pnfpb_push_notification_schedule_checkbox"]) &&
	$_POST["pnfpb_push_notification_schedule_checkbox"] !== "1")) &&
	$post->ID
) {

    update_post_meta(
        $post->ID,
        "pnfpb_push_notification_schedule_checkbox",
        ""
    );
    ...
    if (
        ...
        false === as_has_scheduled_action("PNFPB_cron_post_hook")
    ) {
        $from = get_bloginfo("name");

        $post_type = $post->post_type;

        $post_meta_box_push_notification = get_post_meta(
            $post->ID,
        ...
        if (
            (isset($_POST["pnfpb_push_notification_send_checkbox"]) &&
                sanitize_text_field(
                    wp_unslash(
                        $_POST["pnfpb_push_notification_send_checkbox"] === "1"
                    )
                )) ||
            ($post_meta_box_push_notification === "1") ||
            ...
            if (isset($post->post_status)) {
                if ($post->post_status == "publish") {
					...
					if (
						$post_type === "reply" &&
						function_exists("bbp_get_reply_url")
					) {
						$postlink = bbp_get_reply_url($post->ID);
					} else {
						if (
							$post_type === "topic" &&
							function_exists("bbp_get_topic_permalink")
						) {
							$postlink = bbp_get_topic_permalink($post->ID);
						} else {
							if (
								$post_type === "forum" &&
								function_exists("bbp_get_forum_permalink")
							) {
					...
					if ( (defined( 'DISABLE_WP_CRON' ) && DISABLE_WP_CRON) || get_option("pnfpb_ic_fcm_async_notifications") === false || get_option("pnfpb_ic_fcm_async_notifications") !== "1") {
						...
						$this->PNFPB_icforum_push_notifications_post_web(
							$post->post_title,
							$post->post_content,
							$postlink,
							$post->ID,
						...
						// WP-Cron is enabled
						$arguments = array(	$post->post_title,
										   $post->post_content,
										   $postlink,
										   $post->ID,
										   $post 
										  );
						as_schedule_single_action( time()+2, 'PNFPB_trigger_post_notification_action',$arguments );
					}
                }
            }
        }
	...
	if (
		isset($_POST["pnfpb_push_notification_schedule_checkbox"]) &&
		$_POST["pnfpb_push_notification_schedule_checkbox"] === "1" &&
		isset($_POST["pnfpb_ic_fcm_token_ondemand_datepicker_post"]) &&
		$_POST["pnfpb_ic_fcm_token_ondemand_datepicker_post"] !== "" &&
		$post->ID
	) {
		$pnfpb_push_notification_schedule_checkbox = sanitize_text_field(
			wp_unslash($_POST["pnfpb_push_notification_schedule_checkbox"])
		);

		$pnfpb_ic_fcm_token_ondemand_datepicker_post = sanitize_text_field(
			wp_unslash($_POST["pnfpb_ic_fcm_token_ondemand_datepicker_post"])
		);

		$pnfpb_ic_fcm_token_ondemand_timerpicker_post = "";

		if (isset($_POST["pnfpb_ic_fcm_token_ondemand_timepicker_post"])) {
			$pnfpb_ic_fcm_token_ondemand_timerpicker_post = sanitize_text_field(
				wp_unslash($_POST["pnfpb_ic_fcm_token_ondemand_timepicker_post"])
			);
		}
		...
		update_post_meta(
			$post->ID,
			"pnfpb_push_notification_schedule_checkbox",
			$_POST["pnfpb_push_notification_schedule_checkbox"]
		);
		...
		if ($_POST["pnfpb_ic_fcm_token_ondemand_timepicker_post"] === "") {
			$selected_day_push_notification =
				$pnfpb_ic_fcm_token_ondemand_datepicker_post . "00:00";
		}
		...
		if (
			isset($_POST["pnfpb_ic_fcm_token_ondemand_repeat_month_post"]) &&
			($_POST["pnfpb_ic_fcm_token_ondemand_repeat_month_post"] != "" ||
				$_POST["pnfpb_ic_fcm_token_ondemand_repeat_day_post"] != "")
		) {
			$pnfpb_ic_fcm_token_ondemand_repeat_month_post = sanitize_text_field(
				wp_unslash($_POST["pnfpb_ic_fcm_token_ondemand_repeat_month_post"])
			);

			$pnfpb_ic_fcm_token_ondemand_repeat_day_post = sanitize_text_field(
				wp_unslash($_POST["pnfpb_ic_fcm_token_ondemand_repeat_day_post"])
			);

			$pnfpb_ic_fcm_token_ondemand_repeat_day_number_post = "";

			if (
				isset($_POST["pnfpb_ic_fcm_token_ondemand_repeat_day_number_post"])
			) {
				$pnfpb_ic_fcm_token_ondemand_repeat_day_number_post = sanitize_text_field(
					wp_unslash(
						$_POST["pnfpb_ic_fcm_token_ondemand_repeat_day_number_post"]
					)
				);
			}
			...
			if ($pnfpb_ic_fcm_token_ondemand_repeat_month_post === "") {
				...
				$selected_recurring_month_status .= esc_html(
					__("Every month ", "push-notification-for-post-and-buddypress")
				);
			} else {
			...
			if ($pnfpb_ic_fcm_token_ondemand_repeat_day_post === "") {
				...
				$selected_recurring_day_status .= esc_html(
					__(" (any day)", "push-notification-for-post-and-buddypress")
				);
			} else {
			...
			$data = [
				...
				"content" => mb_substr(
					stripslashes(
						wp_strip_all_tags(
							urldecode(
								trim(htmlspecialchars_decode($post->post_content))
							)
						)
					),
					0,
			...
			if ($action_scheduler_status) {
				$pnfpb_admin_message =
					esc_html(
						__(
							"Push notification scheduled for this post. ",
							"push-notification-for-post-and-buddypress"
						)
					) .
					esc_html(
						__(
							"Action scheduler task id is ",
							"push-notification-for-post-and-buddypress"
						)
					) .
					$action_scheduler_status;
			} else {
				$pnfpb_admin_message = esc_html(
					__(
						"Error in  scheduling push notification for this post, Please try again ",
						"push-notification-for-post-and-buddypress"
					)
				);
			}
			...
			if ($action_scheduler_status) {
				$pnfpb_admin_message =
					...
					esc_html(
						__(
							"Scheduled timestamp reference number is ",
							"push-notification-for-post-and-buddypress"
						)
					) .
					$scheduled_day_push_notification .
