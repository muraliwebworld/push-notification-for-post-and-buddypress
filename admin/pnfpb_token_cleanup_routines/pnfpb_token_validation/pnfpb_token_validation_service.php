...
if ( ! class_exists( 'PNFPB_Token_Validation_Service' ) ) {
	...
	class PNFPB_Token_Validation_Service {
		...
		public static function move_to_trash( $record, $reason, $source, $run_id = 0 ) {
			...
			$inserted = $wpdb->insert( $tables['trash'], array( 'original_id' => $id, 'userid' => absint( $record['userid'] ?? 0 ), 'device_id' => $record['device_id'], 'subscription_option' => $record['subscription_option'] ?? null, 'ip_address' => $record['ip_address'] ?? null, 'web_auth' => $record['web_auth'] ?? null, 'web_256' => $record['web_256'] ?? null, 'subscription_auth_token' => $record['subscription_auth_token'] ?? null, 'firebase_version' => $record['firebase_version'] ?? 'L', 'removal_reason' => sanitize_text_field( $reason ), 'removal_source' => sanitize_key( $source ), 'last_validation_error' => sanitize_text_field( $reason ), 'removed_at' => current_time( 'mysql' ) ), array( '%d', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s' ) );
			if ( false === $inserted ) {
