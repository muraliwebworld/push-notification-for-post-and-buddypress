...
if (!class_exists("PNFPB_ICFM_browser_delivery_notifications_List")) {
    class PNFPB_ICFM_browser_delivery_notifications_List extends WP_List_Table
    {
        public $pre_name = "PNFPB_browser_delivery_list_";
        ...
        public function __construct()
        {
            parent::__construct([
                "singular" => __(
                    "BrowserDeliveredNotification",
                    "push-notification-for-post-and-buddypress"
                ), //singular name of the listed records
                "plural" => __(
                    "BrowserDeliveredNotifications",
                    "push-notification-for-post-and-buddypress"
         ...
         * @return mixed
         */
        public static function get_browser_delivery_notifications(
            $per_page = 20,
            $page_number = 1,
            $search = "",
            ...
            if (
                ...
                $sql = "SELECT * FROM {$wpdb->prefix}pnfpb_ic_delivery_statistics_notifications WHERE notificationid = {$search} OR title = {$search} OR content LIKE '%{$search}%'";
            } else {
                if (
                    !empty($search) &&
                    !is_numeric($search) 
                ) {
                    $sql = "SELECT * FROM {$wpdb->prefix}pnfpb_ic_delivery_statistics_notifications WHERE title LIKE '%{$search}%' OR content LIKE '%{$search}%'";
                } else {
                    $sql = "SELECT * FROM {$wpdb->prefix}pnfpb_ic_delivery_statistics_notifications";
				} 
            }
         ...
         */
        public static function delete_browser_delivery_notification(
            $id
        ) {
            global $wpdb;
        ...
        public static function record_count()
        {
            ...
            $sql = "SELECT COUNT(*) FROM {$wpdb->prefix}pnfpb_ic_delivery_statistics_notifications";
        ...
        {		
			if (isset($_REQUEST["_wpnonce"]) && !wp_verify_nonce(sanitize_text_field(wp_unslash($_REQUEST["_wpnonce"])), "pnfpb_search_browser_delivery_pushnotification")) {
				die("wnonce failure");
			} else {
				//data
				if (isset($_REQUEST["s"])) {
				...
				if (isset($_REQUEST["s"])) {
					$this->items = self::get_browser_delivery_notifications(
						$per_page,
						$current_page,
						sanitize_text_field(wp_unslash($_REQUEST["s"])),
						""
					);
					$total_items_search = self::get_browser_delivery_notifications(
						0,
						$current_page,
						sanitize_text_field(wp_unslash($_REQUEST["s"])),
						""
        ...
        public function pnfpb_url_scheme(
            ...
            if (
                !empty($url) &&
                mb_strpos($url, "?page=pnfpb_icfm_browser_delivery_notifications_list") !==
                    false &&
                isset($_REQUEST["s"])
            ) {
                $search_nonce = wp_create_nonce(
                    "pnfpb_search_browser_delivery_pushnotification"
                );
                ...
                if (!wp_verify_nonce($delete_nonce, "pnfpb_search_browser_delivery_pushnotification")) {
					...
					foreach ($delete_ids as $id) {
						...
						if (count($notifications) > 0) {
							...
							self::delete_browser_delivery_notification(
								absint($id)
							);
						}
					}
