...
?>
	...
	?>
		...
		<form action="options.php" method="post" enctype="multipart/form-data" class="form-field">
         ...
         $pnfpb_swresponse = wp_remote_head(
             home_url("/") . "pnfpb_icpush_pwa_sw.js",
             ["sslverify" => false]
         );

         $pnfpb_swresponse_code = wp_remote_retrieve_response_code($pnfpb_swresponse);

         if (200 === $pnfpb_swresponse_code) {
             $pnfpb_createfileresult = $wp_filesystem->delete(
                 trailingslashit(get_home_path()) . "pnfpb_icpush_pwa_sw.js"
             );
         }

         $pnfpb_firebase_swresponse = wp_remote_head(
             home_url("/") . "firebase-messaging-sw.js",
             ["sslverify" => false]
         );

         $pnfpb_firebase_swresponse_code = wp_remote_retrieve_response_code(
             $pnfpb_firebase_swresponse
         );

         if (200 === $pnfpb_firebase_swresponse_code) {
             $pnfpb_createfileresult = $wp_filesystem->delete(
         ...
         if (get_option("pnfpb_ic_pwa_app_enable") === "1") {
             $pnfpb_pwa_manifest_response = wp_remote_head(
                 home_url("/") . "pnfpbmanifest.json",
                 ["sslverify" => false]
             );

             $pnfpb_pwa_manifest_response_code = wp_remote_retrieve_response_code(
                 $pnfpb_pwa_manifest_response
             );

             if (200 === $pnfpb_pwa_manifest_response_code) {
                 $pnfpb_createfileresult = $wp_filesystem->delete(
