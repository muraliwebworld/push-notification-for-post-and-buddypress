//
//Script to send push notification using web-push
//
//@since 1.0.0
//
"use strict";
...
var data = {
	...
	pushtype: 'icwebpushcred'
};

$j.post(pnfpb_ajax_object_push.ajax_url, data, async function(responseajax) {

	var webpushcredentialsobject = JSON.parse(responseajax);

	vapidKey = '';

	if (webpushcredentialsobject.vapidKey && webpushcredentialsobject.vapidKey != '') {

		vapidKey = webpushcredentialsobject.vapidKey;

	}
		
	pnfpb_ic_custom_click_action_url = '';
...
if (pnfpb_ajax_object_push.pwaapponlyenable === '1') {
	...
	else
	{
		...
		if ('serviceWorker' in navigator) {
			...
			navigator.serviceWorker.register(homeurl+'/pnfpb_icpush_pwa_sw.js',{scope:homeurl+'/'}).then(async function(registration) {
				...
				if ('serviceWorker' in navigator && 'PushManager' in window && ((pnfpb_ajax_object_push.notify_loggedin === '1' && isuserloggedin) || (pnfpb_ajax_object_push.notify_loggedin !== '1') )) {
					...
					navigator.serviceWorker.ready.then(async (serviceWorkerRegistration) => {
						...
						if (pnfpb_ajax_object_push.pnfpb_push_prompt !== '1' && (!$j(".pnfpb-push-subscribe-icon-shortcode").length)) {
	...
	async function pnfpb_show_permission_dialog(registration,subscription) {
		...
		if ((pnfpb_ajax_object_push.pnfpb_ic_fcm_prompt_on_off === '1' && (pnfpb_ajax_object_push.pnfpb_ic_fcm_prompt_style === '1' || pnfpb_ajax_object_push.pnfpb_ic_fcm_prompt_style === '2' )) && pnfpb_ajax_object_push.pnfpb_ic_fcm_prompt_style3 !== '1' ) {
			...
			for (let i = 0; i < ca.length; i++) {
				let c = ca[i].trim(); // Cleaner way to remove leading spaces
...
async function shortcode_subscription_menu(refreshedToken) {
    if( $j(".pnfpb-push-subscribe-icon-shortcode").length )
    {
...
async function requestPermission(registration,shortcode_field='',subscription_options='10000000000000') {
	...
	if (!("Notification" in window)) {
	  ...
	  } else if (Notification.permission === "denied" && (pnfpb_ajax_object_push.pnfpb_push_prompt === '1' || ($j(".pnfpb-push-subscribe-icon-shortcode").length))) {
 ...
 async function checkdeviceid(registration,subscription) {
		...
		if (!pnfpb_webview && 'serviceWorker' in navigator ) {
